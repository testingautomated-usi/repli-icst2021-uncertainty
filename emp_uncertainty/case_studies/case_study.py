...
class CaseStudy(abc.ABC):
    ...
    def run_nn_inference(self,
        ...
        if self.stochastic_model is not None:
            ...
            r_val = self._stochastic_predict_quantified(val_dataset[0])
            ...
            for ood_severity, ood_x in oods_x.items():
                r = self._stochastic_predict_quantified(ood_x)
    ...
    def _stochastic_predict_quantified(self, x):
        batch_size = self.get_batch_size()
        return self.stochastic_model.predict_quantified(x=x,
    ...
    @classmethod
    def calculate_thresholds(cls, y_labels: np.ndarray, model_type: str, epoch: Optional[int]):
    ...
    def _create_stochastic_model(
            cls) -> uwiz.models.StochasticSequential:  # TODO switch to uwiz.models.Stochastic once exposed in uwiz
        pass

    @classmethod
    @abc.abstractmethod
    def _create_ensemble_model(
            cls) -> Union[None, uwiz.models.LazyEnsemble]:  # TODO switch to uwiz.models.Ensemble once exposed in uwiz
        pass
...
class ClassificationCaseStudy(CaseStudy, abc.ABC):
    ...
    def _eval_all(cls, nn_outputs, quantifiers, true_labels, sample_size: Union[int, None], model_type: str,
        ...
        for q_name, q in quantifiers.items():
            results.append(cls._eval_classification_quantifier(quantifier_name=q_name,
                                                               quantifier=q,
    ...
    @classmethod
    def _eval_classification_quantifier(cls,
                                        quantifier_name: str,
    ...
    def _evaluate_stochastic(cls,
                             ...
                             np.ndarray, nn_outputs: np.ndarray,
                             thresholds: Dict[str, float]) -> Iterable[Result]:
        print(f"Calculating quantifications for stochastic model (epoch {epoch})")
    ...
    def _evaluate_ensemble(cls,
                           ...
                           np.ndarray, nn_outputs: np.ndarray,
                           thresholds: Dict[str, float]) -> Iterable[Result]:
        print(f"Calculating quantifications for ensemble model (epoch {epoch})")
        list_of_lists = []
        for s_size in range(2, nn_outputs.shape[1]):
            list_of_lists.append(cls._eval_all(nn_outputs[:, :s_size, :],
                                               quantifiers=BAYES_CLASSIFICATION_QUANTIFIERS,
                                               true_labels=true_labels,
                                               sample_size=s_size,
                                               thresholds=thresholds,
